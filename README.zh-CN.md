...
> 任何数据**、**绝不自动提交**——而且**以文字系统无关为设计支点**：**每种
> 文字系统都是一等公民**——汉字（CJK）、拉丁、西里尔、阿拉伯、希伯来、天城文、
> 希腊文……应有尽有。你带**任意格式、任意语言**的简历进来，得到干净的 LaTeX /
> PDF / DOCX，其中 **CJK 字体排版像素级精确**（这是我们把控得最好的硬骨头——只要
> xelatex + CJK 字体能跑通，其余只是字体与双向排版的事，而非重写），全程自己掌控。
> 配你已经在用的 AI 编辑器即可（Cursor / Codex / WorkBuddy）。
