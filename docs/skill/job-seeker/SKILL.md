...
# job-seeker — 申职器 Resume Workflow Skill (Portable Export)

> **Portable export.** This file is a portable mirror of the WorkBuddy-native skill at `.workbuddy/skills/job-seeker/SKILL.md`.
>
> - To use in **WorkBuddy**: copy this directory to `.workbuddy/skills/job-seeker/` and add `agent_created: true` to the frontmatter above.
> - To use in **Cursor**: copy this directory to `.cursor/skills/job-seeker/`. Cursor ignores the `agent_created` field.
> - To use in a **generic editor**: just open this `SKILL.md` and follow the workflow manually.
...
When generating **example** content, always use placeholders:
